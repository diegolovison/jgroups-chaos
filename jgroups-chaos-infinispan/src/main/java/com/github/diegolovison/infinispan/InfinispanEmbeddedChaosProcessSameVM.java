...
public class InfinispanEmbeddedChaosProcessSameVM extends InfinispanChaosProcess {
   ...
   public void waitForClusterToForm(int numberOfNodes) {
      boolean run = Eventually.runUntil(() -> {
         return getMembersSize() == numberOfNodes;
      }, 30);
      if (!run) {
         int size = getMembersSize();
         throw new IllegalStateException(String.format("number of nodes %d is not the expected: %d", size, numberOfNodes));
      }
   }

   private int getMembersSize() {
      int size = -1;
      JChannel channel = this.getJChannel();
      if (channel != null) {
         size = this.getJChannel().getView().getMembers().size();
      }
      return size;
   }
